...
func (t Training) distance() float64 {
	...
	trainingDistance := float64(t.Action) * t.LenStep / MInKm
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	runningCalories := ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) *
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	walkingCalories := (CaloriesWeightMultiplier*w.Weight + ((math.Pow(w.meanSpeed()*KmHInMsec, 2))/
		(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
...
func (s Swimming) meanSpeed() float64 {
	...
	averageSpeed := float64(s.LengthPool) * float64(s.CountPool) / float64(MInKm) / float64(s.Duration.Hours())
	return float64(averageSpeed)
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	swimmingCalories := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) *
